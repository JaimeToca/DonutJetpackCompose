...
private const val ANIM_DURATION_IN_MILLIS = 20000
...
class MainActivity : AppCompatActivity() {
    ...
    fun RotatingDonut(modifier: Modifier) {
        val animatedProgressA = remember { Animatable(0f) }
        val animatedProgressB = remember { Animatable(0f) }

        LaunchedEffect(animatedProgressA){
            animatedProgressA.animateTo(
                targetValue = TWO_PI,
        ...
        LaunchedEffect(animatedProgressB){
            animatedProgressB.animateTo(
                targetValue = TWO_PI,
                animationSpec = infiniteRepeatable(
                    animation = tween(
                        durationMillis = ANIM_DURATION_IN_MILLIS,
        ...
        Canvas(modifier = modifier, onDraw = {
            val cosA = cos(animatedProgressA.value)
            val cosB = cos(animatedProgressB.value)
            val sinA = sin(animatedProgressA.value)
            val sinB = sin(animatedProgressB.value)
